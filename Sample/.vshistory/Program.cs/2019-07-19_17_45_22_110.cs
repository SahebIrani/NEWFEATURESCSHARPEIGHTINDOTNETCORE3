	...
	public class Program
	{
		...
		public static async Task Main(string[]? args)
		{
			...
			IAsyncEnumerable<string?> lines =
				ReadAllLines(args.Any() ? args[0] : "doc.txt");

			IAsyncEnumerable<string?> res = from line in lines where line.Length > 0 select line;

			IAsyncEnumerable<string?> res2 = lines.Where(c => c.Length > 0);

			IAsyncEnumerable<string?> res3 =
				lines.WhereAwait(line => CheckLine(line))
			;

			IAsyncEnumerable<string?> res4 =
			...
			await res2.ForEachAsync(line => Console.WriteLine(line));

			await res3.ForEachAwaitAsync(line => LinePrint(line));

			await res4.ForEachAwaitWithCancellationAsync(
