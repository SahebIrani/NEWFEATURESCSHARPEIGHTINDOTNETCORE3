...
namespace Simple
{
	public class Program
	{
		...
		private static async ValueTask<bool> CheckLine(string line)
			=> await Task.Run(() => line.StartsWith("Project"));
		...
		public static async Task Main(string[]? args)
		{
			...
			var q = lines.Where(c => c.StartsWith("Project"));

			IAsyncEnumerable<string?> res2 =
				lines.WhereAwait(line => CheckLine(line))
				.Select(line => line.Substring("Project".Length))
			;
			...
			await lines.ForEachAsync(line => Console.WriteLine(line));

			await lines.ForEachAwaitAsync(line => LinePrint(line));

			await lines.ForEachAwaitWithCancellationAsync(
